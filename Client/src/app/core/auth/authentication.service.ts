...
export class AuthenticationService implements AuthService {
	...
	API_ENDPOINT_REGISTER = '/auth/register';
	...
	public login(credential: Credential): Observable<any> {
		...
		return this.http.post<any>(this.API_URL + this.API_ENDPOINT_LOGIN, credential).pipe(
			map((result: any) => {
				if (!result.success) {
					let ms = '请重试';
					if (result.allMessages) {
						ms = result.allMessages;
					}
					this.notifyService.error('登陆失败', ms);
					return result;
				}
	...
	public register(credential: Credential): Observable<any> {
		...
		return this.http.post<any>(this.API_URL + this.API_ENDPOINT_REGISTER, credential)
			.pipe(
				map(result => {
					return result;
				}),
